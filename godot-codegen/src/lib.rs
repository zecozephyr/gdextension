...
pub fn generate_sys_files(sys_gen_path: &Path, build_config: &str) {
    let mut out_files = vec![];
    let mut watch = StopWatch::start();

    generate_sys_mod_file(sys_gen_path, &mut out_files);

    let api = load_extension_api(&mut watch);
...
pub fn generate_core_files(core_gen_path: &Path, build_config: &str) {
    let mut out_files = vec![];
    let mut watch = StopWatch::start();
